...
import { ArrowLeft, Download, FileWarning, Loader2, RefreshCw, Share2, X, ZoomIn, ZoomOut } from "lucide-react";
...
async function fetchBlob(doc: ViewerDoc): Promise<{ blob: Blob; mime: string; bytes: Uint8Array }> {
  ...
  const blob = new Blob([buf], { type: doc.mime_type || res.headers.get("content-type") || "application/octet-stream" });
  let mime = doc.mime_type || blob.type || "application/octet-stream";
  if ((mime === "application/octet-stream" || !mime) && doc.file_name?.toLowerCase().endsWith(".pdf")) mime = "application/pdf";
...
export function TBDocumentViewer({
  ...
  return (
    <div className="flex h-full flex-col bg-background">
        ...
        {state === "error" && (
          <div className="mx-auto flex max-w-md flex-col items-center gap-3 p-8 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              <Button size="sm" onClick={retry}><RefreshCw className="mr-1 h-4 w-4" />Tentar novamente</Button>
              <Button variant="outline" size="sm" onClick={download}><Download className="mr-1 h-4 w-4" />Baixar documento</Button>
              <Button variant="ghost" size="sm" onClick={onBack}><ArrowLeft className="mr-1 h-4 w-4" />Voltar aos documentos</Button>
            </div>
      ...
      <footer
        className="sticky bottom-0 z-10 flex items-center gap-2 border-t border-border bg-background/95 px-2 backdrop-blur"
        style={{ paddingTop: "0.5rem", paddingBottom: "max(0.5rem, env(safe-area-inset-bottom))" }}
      >
        {isPdf && (
          <div className="flex items-center gap-1">
            <Button variant="outline" size="sm" onClick={() => setZoom((z) => Math.max(0.5, +(z - 0.25).toFixed(2)))} disabled={state !== "ready"} aria-label="Diminuir zoom" className="min-h-[44px] min-w-[44px]">
              <ZoomOut className="h-4 w-4" />
            </Button>
            <span className="w-12 text-center tabular-nums text-xs text-muted-foreground">{Math.round(zoom * 100)}%</span>
            <Button variant="outline" size="sm" onClick={() => setZoom((z) => Math.min(3, +(z + 0.25).toFixed(2)))} disabled={state !== "ready"} aria-label="Aumentar zoom" className="min-h-[44px] min-w-[44px]">
              <ZoomIn className="h-4 w-4" />
            </Button>
          </div>
        )}
        <div className="ml-auto grid flex-1 grid-cols-2 gap-2 sm:flex sm:flex-none">
