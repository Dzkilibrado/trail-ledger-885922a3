...
function makeSupportCode(requestId: string): string {
  const compact = requestId.replace(/[^a-z0-9]/gi, "").toUpperCase();
  return `SR-${compact.slice(0, 8).padEnd(8, "0")}`;
}

function maskUserId(userId: string | null | undefined): string | null {
...
function sanitizeReceiptPayloadForLog(receipt: Record<string, unknown> | null | undefined) {
