...
export function aggregateConfidence(snapshot: CockpitSnapshot): string {
  ...
  const avg =
    evaluated.reduce((acc, c) => acc + (CONF_WEIGHT[c.diagnosis.confidence.level] ?? 0), 0) /
    evaluated.length;
...
export function checkEmission(input: {
  ...
  if (!snapshot) {
    ...
    return {
      ...
      conflicts,
      missingData,
      confidenceLevel: "not_evaluable",
    };
  }

  const hasIdentification = !!moto.chassis;
  if (!hasIdentification) {
    blockers.push({
      code: "no_identification",
      title: "Motocicleta sem chassi informado",
      fix: "Informe o chassi no cadastro da moto. Motos de trilha, motocross e uso fora de estrada normalmente não têm placa nem Renavam — o chassi (número de série do quadro) é o identificador exigido para o laudo.",
    });
  }
  ...
  if (confidenceLevel === "medium")
    reservations.push(
      "A confiabilidade geral da análise é média: alguns registros importantes estão ausentes.",
    );
  if (confidenceLevel === "low")
    reservations.push(
      "A confiabilidade geral da análise é baixa: existem poucos dados ou informações desatualizadas.",
    );
  if (confidenceLevel === "not_evaluable")
    reservations.push("Não há dados suficientes para avaliar a maior parte dos componentes.");

  const unknown = evaluated.filter((c) => c.diagnosis.status === "unknown");
  if (unknown.length > 0) {
    reservations.push(
      `${unknown.length} componente(s) sem informação suficiente para diagnóstico.`,
    );
    for (const c of unknown)
      missingData.push(
        `${c.name}: ${c.diagnosis.confidence.bestNextAction ?? "informe a última manutenção"}`,
      );
  }

  for (const c of evaluated) {
    if (c.diagnosis.hasConflict)
      conflicts.push(`${c.name}: informações conflitantes entre horas, quilometragem e datas.`);
  }
  if (conflicts.length > 0)
    reservations.push("Foram detectadas informações conflitantes em um ou mais componentes.");

  const last = snapshot.stats.lastActivityAt
    ? new Date(snapshot.stats.lastActivityAt).getTime()
    : null;
  ...
  const mode: EmissionMode =
    blockers.length > 0 ? "blocked" : reservations.length > 0 ? "with_reservations" : "normal";
