...
function DocumentsHub() {
  const q = useQuery({
    ...
    queryFn: async () => {
      ...
      const byMoto: Record<
        string,
        { total: number; hasInvoice: boolean; last: string | null; types: Set<string> }
      > = {};
      for (const d of rows) {
        if (!d.is_current || d.deleted_at) continue;
        const e = (byMoto[d.motorcycle_id] ??= {
          total: 0,
          hasInvoice: false,
          last: null,
          types: new Set(),
      ...
      {q.isLoading ? (
        ...
        <div className="surface-elevated grid place-items-center rounded-2xl p-10 text-center">
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            Cadastre uma motocicleta para começar a documentar.
          </p>
        </div>
      ) : (
        <div className="grid gap-3 sm:grid-cols-2">
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="truncate font-semibold">
                  {m.nickname ?? `${m.brand ?? ""} ${m.model ?? ""}`.trim()}{" "}
                  <span className="text-muted-foreground">· {m.year_model ?? ""}</span>
                </div>
                <div className="text-[11px] text-muted-foreground">
                  <span className="font-mono">{m.trailbook_id ?? "—"}</span>
                </div>
                <div className="mt-1 flex flex-wrap items-center gap-2 text-[11px]">
                  <span className="rounded-md bg-muted px-1.5 py-0.5 font-bold">
                    {m.stats.total} docs
                  </span>
                  {m.stats.hasInvoice && (
                    <span className="flex items-center gap-1 text-emerald-400">
                      <ShieldCheck className="h-3 w-3" /> NF
                    </span>
                  )}
                  {m.stats.missing.length > 0 && (
                    <span
                      className="flex items-center gap-1 text-amber-400"
                      title={`Pendentes: ${m.stats.missing.map((t) => DOC_TYPE_LABEL[t as never]).join(", ")}`}
                    >
                      <AlertCircle className="h-3 w-3" /> {m.stats.missing.length} pendente
                      {m.stats.missing.length > 1 ? "s" : ""}
                    </span>
                  )}
                  {m.stats.last && (
                    <span className="text-muted-foreground">
                      · atualizado em {new Date(m.stats.last).toLocaleDateString("pt-BR")}
                    </span>
                  )}
                </div>
              </div>
