...
function AdminUsers() {
  ...
  async function quickReactivate(uid: string) {
    const { error } = await supabase.rpc("admin_reactivate_user" as any, {
      _user: uid,
      _notes: null,
    });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Usuários"
        description="Gerencie contas, planos e status de acesso."
            ...
            {(users.data ?? []).map((u) => {
              ...
              return (
                <TableRow key={u.id}>
                  ...
                  <TableCell>
                    <Badge className={STATUS_TONE[u.status]}>{u.status}</Badge>
                  </TableCell>
                  <TableCell className="text-xs">
...
function UserDetailsSheet({
  ...
  const audit = useQuery({
    ...
    enabled: !!userId,
    queryFn: async () => {
      const { data, error } = await supabase.rpc("admin_user_audit" as any, {
        _user: userId,
        _limit: 100,
      });
      if (error) throw error;
          ...
          <SheetHeader className="space-y-4 text-left">
            <div className="flex items-start gap-4 pr-8">
              ...
              <div className="min-w-0 flex-1 space-y-2">
                <div>
                  <SheetTitle className="break-words text-xl leading-tight">
          ...
          {!details.isError && profile && (
            <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-5">
              <SummaryCard
                icon={<Bike />}
                label="Motos"
                value={row?.motorcycles_count ?? motorcycles.length ?? 0}
              />
              <SummaryCard
                icon={<FileText />}
                label="Documentos"
                value={row?.documents_count ?? documents.length ?? 0}
              />
              <SummaryCard
                icon={<ScrollText />}
                label="Certificados"
                value={row?.certificates_count ?? certificates.length ?? 0}
              />
              <SummaryCard
                icon={<Ticket />}
                label="Chamados"
                value={`${row?.open_tickets ?? openTickets ?? 0} aberto(s)`}
              />
              <SummaryCard
                icon={<Clock />}
                label="Último acesso"
                value={profile?.last_seen_at ? formatDate(profile.last_seen_at) : "Sem registro"}
              />
            </div>
          )}
        </div>
            ...
            <Tabs defaultValue="dados" className="space-y-4">
              ...
              <TabsContent value="dados" className="space-y-5">
                <SectionTitle
                  ...
                  description="Informações principais da conta e do acesso."
                />
                ...
                <div className="space-y-3 rounded-lg border border-border p-4">
                  <SectionTitle
                    ...
                    description="Dados, perfil e plano. Alterações relevantes geram auditoria."
                  />
                  <EditForm
                    profile={profile}
                    row={row}
              ...
              <TabsContent value="motos" className="space-y-4">
                <SectionTitle
                  ...
                  description="Motocicletas cadastradas pelo usuário."
                />
                {motorcycles.length === 0 ? (
                  <EmptyState>Este usuário ainda não cadastrou nenhuma motocicleta.</EmptyState>
                ) : (
                  <div className="space-y-2">
                    {motorcycles.map((m) => (
                      <ListItem
                        key={m.id}
                        title={formatMotorcycleName(m)}
                        meta={`${m.trailbook_id || "Sem TrailBook ID"}${m.plate ? ` · Placa ${m.plate}` : ""}`}
                      />
                    ))}
                  </div>
                )}
              ...
              <TabsContent value="documentos" className="space-y-4">
                <SectionTitle
                  title="Documentos"
                  description="Arquivos e registros documentais das motos do usuário."
                />
                {documents.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {documents.map((doc) => (
                      <ListItem
                        key={doc.id}
                        title={doc.file_name || doc.doc_type || "Documento"}
                        meta={`${doc.motorcycle_label || "Moto"} · ${doc.doc_date ? formatDate(doc.doc_date) : formatDate(doc.created_at)}${doc.deleted_at ? " · Excluído" : ""}`}
                      />
                    ))}
                  </div>
                )}
              ...
              <TabsContent value="certificados" className="space-y-4">
                <SectionTitle
                  title="Certificados"
                  description="Certificados públicos emitidos a partir das motos do usuário."
                />
                {certificates.length === 0 ? (
                  <EmptyState>Nenhum certificado emitido.</EmptyState>
                ) : (
                  <div className="space-y-2">
                    {certificates.map((cert) => (
                      <ListItem
                        key={cert.id}
                        title={cert.public_token || "Certificado"}
                        meta={`${cert.motorcycle_label || "Moto"} · ${cert.status || "status indefinido"} · ${formatDate(cert.created_at)}`}
                      />
                    ))}
                  </div>
                )}
              ...
              <TabsContent value="chamados" className="space-y-4">
                <SectionTitle
                  title="Chamados"
                  description="Solicitações de suporte associadas a este usuário."
                />
                {tickets.length === 0 ? (
                  <EmptyState>Nenhum chamado aberto.</EmptyState>
                ) : (
                  <div className="space-y-2">
                    {tickets.map((ticket) => (
                      <ListItem
                        key={ticket.id}
                        title={`${ticket.code || "Chamado"} · ${ticket.title || ticket.subject || "Sem título"}`}
                        meta={`${ticket.status || "status indefinido"} · ${ticket.priority || "prioridade não informada"} · ${formatDate(ticket.created_at)}`}
                      />
                    ))}
                  </div>
                )}
              ...
              <TabsContent value="seguranca" className="space-y-5">
                <SectionTitle
                  title="Ações administrativas"
                  description="Acesso, recuperação de senha, homologação e controles sensíveis."
                />
                {profile.status === "blocked" && (
                  <div className="rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-xs">
                    ...
                    {profile.blocked_notes && (
                      <div className="mt-1 opacity-80">Obs.: {profile.blocked_notes}</div>
                    )}
                  </div>
                )}
                {profile.status === "inactive" && (
                  <div className="rounded-lg border border-border bg-muted p-3 text-xs">
                    ...
                    {profile.inactive_notes && (
                      <div className="mt-1 opacity-80">Obs.: {profile.inactive_notes}</div>
                    )}
                  </div>
                )}
                <div className="space-y-3 rounded-lg border border-border p-4">
                  ...
                  <div className="grid gap-2 sm:grid-cols-2">
                    <ConfirmDialog
                      ...
                      onConfirm={async () => {
                        try {
                          ...
                          toast.success(
                            "Link de redefinição enviado (ou registrado como simulado).",
                          );
                          refetchAll();
                        } catch (e: any) {
                    ...
                    {profile.status !== "inactive" && (
                      <ReasonDialog
                        ...
                        submitVariant="destructive"
                        onSubmit={async (reason, notes) => {
                          const { error } = await supabase.rpc("admin_deactivate_user" as any, {
                            _user: userId,
                            _reason: reason,
                            _notes: notes,
                          });
                ...
                <div className="space-y-3 rounded-lg border border-border p-4">
                  <SectionTitle
                    title="Homologação"
                    description="Limpeza controlada de contas de teste."
                  />
                    ...
                    <Switch
                      ...
                      onCheckedChange={async (v) => {
                        const { error } = await supabase.rpc("admin_update_user" as any, {
                          _user: userId,
                          _is_homologation: v,
                          _reason: "Alteração de flag de homologação",
                        });
                        if (error) return toast.error(error.message);
                        toast.success(
                          v ? "Marcado como homologação" : "Removida flag de homologação",
                        );
                        refetchAll();
                      }}
                    />
              ...
              <TabsContent value="audit" className="space-y-4">
                <SectionTitle
                  title="Auditoria"
                  description="Histórico de ações administrativas registradas."
                />
...
function SummaryCard({
  ...
  return (
    <div className="rounded-lg border border-border bg-muted/30 p-3">
      <div className="flex items-center gap-2 text-xs text-muted-foreground [&_svg]:h-4 [&_svg]:w-4">
        {icon}
        <span>{label}</span>
      </div>
...
function EditForm({
  ...
  async function save() {
    ...
    if (emailChanged && !confirmEmail)
      return toast.error("Confirme a alteração de e-mail marcando a caixa de confirmação");
    setSaving(true);
    const { error } = await supabase.rpc("admin_update_user" as any, {
      _user: profile.id,
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div className="flex items-center gap-3 rounded-md border border-border p-2 sm:col-span-2">
          ...
          <Label className="text-xs">
            Perfil de Administrador {isSelf && isAdmin && "(você não pode se auto-rebaixar)"}
          </Label>
        </div>
        <div className="sm:col-span-2">
        ...
        {emailChanged && (
          <div className="rounded-md border border-amber-500/30 bg-amber-500/10 p-2 text-xs sm:col-span-2">
            <label className="flex items-start gap-2 cursor-pointer">
              ...
              />
              <span>
                Confirmo a alteração de e-mail (isso impacta login e comunicação com o usuário).
              </span>
            </label>
          </div>
        )}
      ...
      <div className="flex flex-col gap-2 pt-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="text-xs text-muted-foreground">
          {changes.length ? (
            <>
              Alterações: <strong>{changes.join(", ")}</strong>
            </>
          ) : (
            "Sem alterações"
          )}
...
function ReasonDialog({
  ...
  return (
    <AlertDialog open={open} onOpenChange={setOpen}>
      <span
        onClick={(e) => {
          if (!(e.target as HTMLElement).closest("button")?.hasAttribute("disabled")) setOpen(true);
        }}
      >
        {trigger}
      </span>
...
function DeleteHomologDialog({
  ...
  userId: string;
  profile: any;
  counts: { motos: number; docs: number; certs: number; tickets: number };
  onDone: () => void;
}) {
  const [open, setOpen] = useState(false);
    ...
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      >
        <Button variant="destructive" size="sm">
          ...
          Excluir usuário de homologação
        </Button>
      </span>
      <AlertDialogContent className="max-w-lg">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Esta ação removerá definitivamente um usuário de HOMOLOGAÇÃO. Essa operação{" "}
            <strong>não poderá ser desfeita</strong>. Use apenas para limpeza de testes e cenários
            de homologação.
          </AlertDialogDescription>
        </AlertDialogHeader>
        ...
        {step === 1 && (
          <div className="space-y-3 text-xs">
            <div className="rounded-md border border-destructive/30 bg-destructive/10 p-3">
              <div>
                <strong>{profile.full_name || "Usuário"}</strong> · {profile.email}
              </div>
              <div className="mt-2 grid grid-cols-2 gap-1">
                <div>
                  Motos: <strong>{counts.motos}</strong>
                </div>
                <div>
                  Documentos: <strong>{counts.docs}</strong>
                </div>
                <div>
                  Certificados: <strong>{counts.certs}</strong>
                </div>
                <div>
                  Chamados: <strong>{counts.tickets}</strong>
                </div>
              </div>
            </div>
            <div>
            ...
            <AlertDialogFooter>
              <AlertDialogCancel>Cancelar</AlertDialogCancel>
              <Button variant="destructive" onClick={() => setStep(2)} disabled={!finalReason}>
                Continuar
              </Button>
            </AlertDialogFooter>
          </div>
