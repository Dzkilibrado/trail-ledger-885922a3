...
import {
  ...
  USE_PROFILES, ACTION_LABEL, SEVERITY_LABEL,
...
function PlanWizard() {
  ...
  async function loadTemplate(withProfile: UseProfile) {
    ...
    if (!t) { toast.error("Nenhum plano padrão disponível ainda."); setRows([]); setLoaded(true); return; }
  ...
  function addRow() {
    setRows((r) => [...r, {
      key: `custom-${Date.now()}`, item_name: "", name: "Novo item — Inspecionar",
      category: "other", action: "inspect", severity: "medium",
      interval_hours: 20, interval_km: null, interval_days: null,
      notes: null, keep: true, sort_order: 999,
    }]);
  }
  ...
  const totals = useMemo(() => ({
    total: rows.length, active: rows.filter((r) => r.keep).length,
  }), [rows]);
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <section className="surface-elevated rounded-2xl p-5 space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          <div className="space-y-1.5">
            <Label className="text-xs uppercase tracking-widest text-muted-foreground">Tipo de uso</Label>
          ...
          {profile === "other" && (
            <div className="space-y-1.5">
              <Label className="text-xs uppercase tracking-widest text-muted-foreground">Descreva o uso *</Label>
              <Input value={profileNote} onChange={(e) => setProfileNote(e.target.value)} placeholder="ex: uso comercial em fazenda" />
            </div>
          )}
        </div>
      ...
      <section className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{totals.active} de {totals.total} itens ativos.</p>
          </div>
        ...
        {!loaded ? (
          ...
          <ul className="space-y-2">
            {rows.map((r, i) => (
              <li key={r.key} className={`surface-elevated rounded-2xl p-4 ${!r.keep ? "opacity-50" : ""}`}>
                <div className="grid gap-3 sm:grid-cols-[auto_minmax(0,1fr)_auto]">
                  <div className="flex items-start pt-1">
                    <Checkbox checked={r.keep} onCheckedChange={(v) => updateRow(i, { keep: !!v })} />
                  </div>
                  <div className="min-w-0 space-y-2">
                    <div className="grid gap-2 sm:grid-cols-[1fr_1fr_1fr]">
                      <div className="space-y-1">
                        <Label className="text-[10px] uppercase tracking-widest text-muted-foreground">Item</Label>
                        <Input
                          value={r.item_name}
                          onChange={(e) => updateRow(i, { item_name: e.target.value, name: `${e.target.value} — ${ACTION_LABEL[r.action]}` })}
                        />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-[10px] uppercase tracking-widest text-muted-foreground">Categoria</Label>
                        <Select value={r.category} onValueChange={(v) => updateRow(i, { category: v as any })}>
                          <SelectTrigger><SelectValue /></SelectTrigger>
                          <SelectContent>
                      ...
                      <div className="space-y-1">
                        ...
                        <Select
                          value={r.action}
                          onValueChange={(v) => updateRow(i, { action: v as PlanAction, name: `${r.item_name} — ${ACTION_LABEL[v as PlanAction]}` })}
                        >
                          <SelectTrigger><SelectValue /></SelectTrigger>
                          <SelectContent>
                            {Object.entries(ACTION_LABEL).map(([v, l]) => <SelectItem key={v} value={v}>{l}</SelectItem>)}
                          </SelectContent>
                        </Select>
                      </div>
                    </div>
                    <div className="grid gap-2 sm:grid-cols-[1fr_1fr_1fr_1fr]">
                      <div className="space-y-1">
                        <Label className="text-[10px] uppercase tracking-widest text-muted-foreground">A cada (h)</Label>
                        <Input type="number" step="0.1" value={r.interval_hours ?? ""} onChange={(e) => updateRow(i, { interval_hours: e.target.value ? Number(e.target.value) : null })} />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-[10px] uppercase tracking-widest text-muted-foreground">A cada (km)</Label>
                        <Input type="number" value={r.interval_km ?? ""} onChange={(e) => updateRow(i, { interval_km: e.target.value ? Number(e.target.value) : null })} />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-[10px] uppercase tracking-widest text-muted-foreground">A cada (dias)</Label>
                        <Input type="number" value={r.interval_days ?? ""} onChange={(e) => updateRow(i, { interval_days: e.target.value ? Number(e.target.value) : null })} />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-[10px] uppercase tracking-widest text-muted-foreground">Severidade</Label>
                        <Select value={r.severity} onValueChange={(v) => updateRow(i, { severity: v as PlanSeverity })}>
                          <SelectTrigger><SelectValue /></SelectTrigger>
                          <SelectContent>
                            {Object.entries(SEVERITY_LABEL).map(([v, l]) => <SelectItem key={v} value={v}>{l}</SelectItem>)}
                          </SelectContent>
                        </Select>
                      </div>
                    </div>
                  ...
                  <div className="flex items-start justify-end">
                    <Button variant="ghost" size="icon" onClick={() => removeRow(i)} aria-label="Remover item">
      ...
      <div className="flex flex-wrap justify-end gap-2">
        <Button variant="ghost" asChild>
          <Link to="/motorcycles/$id" params={{ id }}>Pular por agora</Link>
        </Button>
